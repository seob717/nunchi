...
기준으로 판정하기 위한 그라운드 트루스 기록기다. ziptie 본체 배달 로직에는
전혀 관여하지 않고, source == "compact"일 때 `$ZIPTIE_OBSERVER_LOG`(런별
...
주입되므로, 여기서 뭔가 출력하면 AC 조건의 "ziptie 관여 없음" 전제가
깨진다. 어떤 실패도 exit 0.
"""
...
def main():
    ...
    try:
        if input_data.get("source") == "compact":
            log_path = os.environ.get("ZIPTIE_OBSERVER_LOG")
            if log_path:
